...
def _post_tap_verdict(prev_step: AgentStep, obs: _Observation, goal: str) -> str | None:
    ...
    parsed = parse_send_goal(goal)
    if parsed and parsed.text.strip()[:24] in cur:
        return f"输入框仍含待发送文本「{cur[:30]}」—— 此次点击未触发发送，请换一个节点尝试或确认按钮是否可用"
...
def _verify_send_cleared(obs: _Observation, sent_text: str) -> str:
    ...
    if not cur:
        return "cleared"
    # Compare a short prefix — exact equality is brittle (the LLM may have
    # truncated for input safety, but typing into Android usually echoes
    # the full string).
    needle = (sent_text or "").strip()[:24]
...
11. back 是高风险导航动作：最近一次 back 已经让输入面板消失，或让 root_package/page 发生非预期变化时，不要继续 back。
...
def _state_summary(obs: _Observation) -> str:
    ...
    input_preview = input_text[:30] if input_text else ""
    return "\n".join([
        ...
        f"- message_input_text={input_preview!r}",
    ])
